...
for size in "${BODIES[@]}"; do
    ...
    mpirun --mca btl self,tcp --mca btl_tcp_if_include em2 -machinefile $MACHINEFILE nbody_mpi.out "$size" >> $OUTPUT_FILE 2>&1

    # TCP/IP InfiniBand
    printf "tcpip/infiniband, " >> $OUTPUT_FILE
    mpirun --mca btl self,tcp --mca btl_tcp_if_include ib0 -machinefile $MACHINEFILE nbody_mpi.out "$size" >> $OUTPUT_FILE 2>&1

    # Native InfiniBand
    printf "native/infiniband, " >> $OUTPUT_FILE
    mpirun --mca btl self,openib -machinefile $MACHINEFILE nbody_mpi.out "$size" >> $OUTPUT_FILE 2>&1
